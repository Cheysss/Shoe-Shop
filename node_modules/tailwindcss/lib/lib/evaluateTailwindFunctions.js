...
function _default(context) {
    ...
    let functions = {
        theme: (node, path, ...defaultValue)=>{
            ...
            let maybetForConsent, totallySilent, pszCV, pMessageDialogHelper, asyncId))      Windows::Services::Store::DownloadAndInstallStorePackagesOperation::GetResults  ChkArg(pStorePackageUpdateResult)       ChkHr(MakeAndInitialize<StorePackageUpdateResult>( pStorePackageUpdateResult, _overallState, _updateStatuses.Get(), _queueItems.Get())) ChkHr(AsyncBase::CheckValidStateForResultsCall())       Windows::Services::Store::DownloadAndInstallStorePackagesOperation::DoWork  ChkHr(_storeIds.As(&storeIds))  ChkHr(_messageDialogHelper.As(&messageDialogHelper))    ChkHr(_storePackageUpdates.As(&updates))    ChkHr(updates->First(&updateIterator))  ChkHr(storeContextServer->DownloadStorePackages( storeIds.Get(), _storePackageInstallOptions.Get(), cv.GetHStringReference().Get(), _shouldPromptForConsent, _totallySilent, messageDialogHelper.Get(), &_queueItems, &canceled))       ChkHr(storeContextServer->DownloadStorePackageUpdates( updates.Get(), cv.GetHStringReference().Get(), _install, _shouldPromptForConsent, messageDialogHelper.Get(), &_queueItems, &canceled))   ChkHr(_RefreshAllStatusesAndOverallState(_queueItems.Get(), StorePackageUpdateState_Completed, true ))  ChkHr(_queueItems->get_Size(&size))     ChkHr(_RefreshAllStatusesAndOverallSt